...
describe('account-meter observation (quota attribution instrument)', () => {
  // These exist so a later measurement can tell "the server sent nothing" apart
  // from "we never looked". Every one of them fails if the observation is removed.

  function quotaFrame(overrides: Record<string, unknown> = {}): string {
    return JSON.stringify({
      type: 'codex.rate_limits',
      rate_limits: {
        // Fractional on purpose: /wham/usage reports an integer, and whether THIS
        // path carries more precision is the whole question.
      ...
      metered_limit_name: 'base_model_inference',
      plan_type: 'pro',
      ...overrides,
    });
  }
  ...
  it('captures a rate-limit frame that arrives DURING a response', async () => {
    ...
    expect(observed[0]!.meteredLimitName).toBe('base_model_inference');
  ...
  it('captures a rate-limit frame that arrives with NO request in flight', async () => {
    ...
    // An idle frame belongs to the CONNECTION; attributing it to the finished
    // response would invent a per-request debit that was never reported.
    expect(observed[0]!.requestId).toBeUndefined();
  });
  ...
  it('omits a field the server did not send rather than reporting it as zero', async () => {
    ...
    expect(observed.meteredLimitName).toBeUndefined();
  ...
  it('records meter-bearing headers from the SUCCESSFUL upgrade response', async () => {
    const diagnostics: ResponsesWebSocketDiagnosticEvent[] = [];
    const wsFetch = createResponsesWebSocketFetch(WS_URL, () => {}, {
      onDiagnostic: event => diagnostics.push(event),
    });
    await wsFetch('https://x', { method: 'POST', headers: {}, body: '{}' });
    const socket = lastSocket();
    socket.emit('upgrade', {
      statusCode: 101,
      headers: {
        ...
        'x-codex-primary-reset-at': '1789805434',
        'content-type': 'application/json',
      },
    });

    const observed = diagnostics.find(d => d.event === 'ws_upgrade_headers')!;
    expect(observed).toBeDefined();
    expect(observed.statusCode).toBe(101);
    expect(observed.quotaHeaderCount).toBe(3);
    // Raw strings, so a fractional percent is not rounded away on the way in.
    expect((observed.quotaHeaders as Record<string, string>)['x-codex-primary-used-percent'])
      .toBe('12.5');
    // Unrelated headers are not hoovered up.
    expect((observed.quotaHeaders as Record<string, string>)['content-type']).toBeUndefined();
  });
});
